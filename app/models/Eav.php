...
class Eav extends layout
{
    ...
    public function getEavData($attGroupId) {
        $entityArray=array();
        $sql='SELECT entity_attribute.attribute_id FROM entity_attribute WHERE entity_attribute.attribute_group_id='.$attGroupId.' order by entity_attribute.sort_order;';
//echo $sql;die();
    ...
    public function createModule($moduleName,$isDrop=0) {
        ...
        if ($moduleName){
            ...
            $sql .= '
                ...
                INSERT INTO module_set_group (`module_id`, `attribute_set_id`, `attribute_group_id`) SELECT  '.$this->moduleId.',14,21;
                INSERT INTO entity_attribute (`attribute_group_id`, `attribute_id`) SELECT  21,id FROM attribute WHERE `attribute_code` LIKE \'exp_%\';';
            $execSql = $this->conn()->exec($sql);
        }
