...
if (!isset($_GET['modules_id'])){$_GET['modules_id']='';}
if (!isset($_GET['attribute_set_id'])){$_GET['attribute_set_id']='';}
if (!isset($_GET['attribute_group_id'])){$_GET['attribute_group_id']='';}
...
function getSelect($db,$table,$gid='',$attArray=array(),$attSelectArray=array(),$attParentArray=array()) {
    ...
    $hiddenHtml.= $table=='entity_attribute' ? '<input type="hidden" name="att[attribute_group_id]" value="'.$_GET['attribute_group_id'].'" >':'';
    ...
    foreach($db[0] as $key=>$row){
        ...
        if ($key=='id'){
            ...
            $addSelectHtml .= '<td><select class="form-control" name="att['.$key.']" ><option value="" ></option>';
            foreach($db as $att){
                $addSelectHtml .= '<option value="'.$att['id'].'" >'.$att['attribute_group_name'].':'.$att['id'].'</option>';
            }
            $addSelectHtml .= '</select></td>';
        }else {
        ...
        if ($db[0]['id']){
            ...
            foreach($row as $key=>$value){
                ...
                <input type="hidden" name="att['.$key.']" placeholder="" class="form-control" value="'.$value.'" ></td>';
                    ...
                    $selectHtml .= '<td><select class="form-control" name="att['.$key.']" ><option value="" ></option>';
                    foreach($db as $att){
                        $selected='';
                        if ($att['id']==$row['id']){ continue; }
                        if ($value==$att['id']) $selected='selected';
                        $selectHtml .= '<option '.$selected.' value="'.$att['id'].'" >'.$att['attribute_group_name'].':'.$att['id'].'</option>';
                    }
...
echo '<style>.table>tbody>tr>td, .table>tbody>tr>th, .table>tfoot>tr>td, .table>tfoot>tr>th, .table>thead>tr>td, .table>thead>tr>th {width: 20%;}</style>';
